# Prompts Draft (search_hybrid_v2)

每个 prompt 一个区段, 状态: ⏳ 待审 / ✅ 已定稿 / 🔁 修订中.

## 总览

| # | Prompt | 状态 | 主要改动 |
|---|---|---|---|
| 1 | UNIVERSAL_WORLDVIEW | ✅ 已定稿 | + TASK SEMANTICS / + OPTIMIZATION OBJECTIVE / 删 trade-off 错误段 / 5→8 lever 更新 |
| 2 | WARMSTART_AGENT_SYSTEM | ✅ 已定稿 | 大减: 删"关键"段 (指挥均匀采样元凶) + 删 example arch / 词数 -65% |
| 3 | SCIENTIST_STAGE_A_SYSTEM | ✅ 已定稿 | 中减: 4 行 Stage 对比 → 1 行, 操作枚举精简, body 示例删, # CONSTRAINTS 单行 |
| 4 | SCIENTIST_STAGE_B1_SYSTEM | ✅ 已定稿 | 小减: 末句压缩, OUTPUT example 保留, 砍工程层兜底说明 |
| 5 | SCIENTIST_STAGE_B2_SYSTEM | ✅ 已定稿 | 小减: INPUT 的 JSON example 压缩为字段名清单 |
| 6 | SUPERVISOR_AGENT_SYSTEM | ✅ 已定稿 | 加 budget_progress 输入 (F-PROMPT-004 核心修复) + 删两条哲学/兜底句 |

## Phase 2 实施依赖 (代码 + prompt 联动)

| 改动 | 仅 prompts.py | 还需改 Python |
|---|:---:|---|
| UNIVERSAL_WORLDVIEW v2 | ✅ | — |
| WARMSTART_AGENT_SYSTEM v2 | ✅ | — |
| SCIENTIST_STAGE_A_SYSTEM v2 | ✅ | — |
| SCIENTIST_STAGE_B1_SYSTEM v2 | ✅ | — |
| SCIENTIST_STAGE_B2_SYSTEM v2 | ✅ | — |
| SUPERVISOR_AGENT_SYSTEM v2 | ❌ | `supervisor_agent.py` + `nsga2_search.py` 需注入 budget_progress |

---

## UNIVERSAL_WORLDVIEW — ✅ 已定稿 (第 1 版, 2026-05-20)
...
**Diff vs v1**:
- 删: 末尾 "典型 trade-off: 大数值..." 整段
- 加: `# TASK SEMANTICS` 新段 (光流任务定义 + EPE 单位)
- 加: `# OPTIMIZATION OBJECTIVE` 新段 (最大化 HV)
- 改: `调 NSGA-II 5 个超参 lever` → `8 个超参 lever`
- 改: `~16 代 × 50 = 800` → `16 代 × 50 = 800`

---

## WARMSTART_AGENT_SYSTEM — ✅ 已定稿 (第 1 版, 2026-05-20)

接在 UNIVERSAL_WORLDVIEW 之后:
...
**Diff vs v1**:
- 删: `**关键**: 你给的种群是后续所有代的基因池...` 整段 (用户诊断: 这段在指挥均匀采样, 是 d_v3 per-dim entropy 接近最大的元凶)
- 删: `你怎么平衡多样性与高潜力区域, 完全自决.`
- 删: 2 个 example arch_codes `"2,1,2,1,2,1,0,1,0,1,0"` 等
- 删: rationale 的 `(落盘到 warmstart_diagnostics.json, 不参与下游决策)` 反向激励
- 合并 constraints 2+3

---

## SCIENTIST_STAGE_A_SYSTEM — ✅ 已定稿 (第 1 版, 2026-05-20)
...
**Diff vs v1**:
- 改 (1b): 两阶段对比 4 行 → 1 行 `"你只做归纳, Stage B (独立调用) 才做硬件验证."`
- 改 (2b): 5 种操作枚举段 → 1 行精简版
- 删 (3b): `允许写` 示例段 ("EB0=2 + DB1=2 子网 EPE 中位 4.05...")
- 改 (4b): `# 不要做 Stage B 的事` 段重写成 `# CONSTRAINTS` 单行
- 保留 (5a): INPUT 列表原样

---

## SCIENTIST_STAGE_B1_SYSTEM — ✅ 已定稿 (第 1 版, 2026-05-20)
...
**Diff vs v1**:
- 改 (1b): 末句 `你不直接写最终 insights.md, 也不直接执行任何工具 -- 只规划.` → `只规划, 不执行.`
- 保留 (2c): OUTPUT FORMAT 长 example 完整保留 (含 verifications 多行 escaped Python)
- 保留 (3b): QUERY 规则 5 条分散表述保留
- 删 (4a): CODE 规则末尾的 `失败 (timeout / 非法 import / ...) 都被 coordinator 捕获并传给 Stage B-2` 整条
- 删 (5a): 末段 `每条 insight 是否需要 vela_query / verification / 都不要..., 由你判断. 同一 insight 多 vela_queries 会被合并.`

---

## SCIENTIST_STAGE_B2_SYSTEM — ✅ 已定稿 (第 1 版, 2026-05-20)
...
**Diff vs v1**:
- 保留 (1c): 第 1 段 4 种命运枚举保留原状
- 改 (2b): INPUT 第 2-3 条的 JSON examples 压缩为字段名清单
- 保留 (3b): 格式契约 5 条保留原状
- 保留 (4c): 末段 "由你判断" 保留原状

---

## SUPERVISOR_AGENT_SYSTEM — ✅ 已定稿 (第 1 版, 2026-05-20)
...
**Diff vs v1**:
- 保留 (1b): "完全由你判断" 原样
- 改 (2a): ACTION SPACE intro 3 句 → 1 句
- 保留 (3b)(4b)(5b): Lever 6/7/8 详细描述全部原样
- 删 (6a): INPUT 段尾 `各 metric 的语义由你根据列名判断, 我们不预设"什么算异常".`
- 删 (7a): HARD CONSTRAINTS 末句 `非法值会被工程层拒绝 (其他合法 lever 照常应用), rejection 进 supervisor_log.`
- 加 (8a): INPUT 新增 `budget_progress` 字段 (当前已用/总 evals + 代号/总代数)
- 保留 (9d): reseed 诱惑 不动 (待 v2 phase 4 实测后再决定加 lever 还是硬约束)
- 保留 (10b): insights 消费引导 不加

**⚠️ 工程影响**: 加点 8a (budget_progress) 是 prompt + code 联动改动. supervisor_agent.py 的 `invoke_supervisor_agent` 函数必须在 user_message 里实际注入这两个数值. Phase 2 实施时一并改.
