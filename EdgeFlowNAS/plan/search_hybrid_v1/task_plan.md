...
Phase 5 ✅ Implementation complete (2026-05-05). Ready to submit slurm jobs.
...
| 3 | Scientist Agent ✅ | 三阶段 (A 归纳 / B-1 规划 / B-2 收尾) + agent 自己规划 vela query + sandbox 执行 verification code | ✅ 3 LLM calls / K 代 |
| 4 | Supervisor Agent ✅ | 5-lever 动作空间 (mutation_prob / crossover_prob / per_dim_multiplier / tournament_size / reseed_bottom_pct), 仅合法性校验, agent 自我纠偏 | ✅ 1 LLM call / K 代 |
| 5 | Validation & Paper Artifacts ✅ | 单 seed × 4 组, (a) 复用 search_v3, 3 新 slurm + 分析脚本 + 红线判定 | ❌ 离线分析 |
...
## Phase 3: Scientist Agent ✅ Complete (2026-05-05)

**最终设计: 3-stage workflow, 用户决策在执行中再优化** —— 偏离原 outline 的
"两阶段, 一次调用内完成", 改为 **真正的两个独立 LLM 调用** (Stage A + Stage B-1
+ Stage B-2) 防止 hardware-verifiable 模式被偏向; 关键决策修正: **vela query
target 由 agent 在 Stage B-1 自己规划, coordinator 只是过滤器执行器**.

### 3.1 Sandbox 模块 ✅

`efnas/search/sandbox.py`:
- `validate_imports`: AST 白名单检查 (pandas/numpy/json/math/re/itertools/scipy/scipy.stats/sys)
- `execute_python`: subprocess 隔离 + 30s timeout + tempdir cwd + 限制 env
- `execute_verification`: 在 execute_python 之上 parse stdout 最后一行为 JSON
- 18/18 测试通过 (含 timeout / non-zero exit / 各种 disallowed import 拒绝)

### 3.2 Scientist Agent 三阶段 + Pipeline ✅

`efnas/search/scientist_agent.py`:
- **Stage A** (`invoke_scientist_stage_a`): 纯架构归纳, 输入 history+metrics+
  prev insights+progress, 输出 drafts (id/status/title/body), 不接触 Vela
- **Stage B-1** (`invoke_scientist_stage_b1`): 规划 vela_queries +
  verifications + annotations_no_code; agent **自己决定**要查哪些 archs
- **Resolve vela queries** (`resolve_vela_queries`, 纯 Python): 按 pattern /
  arch_codes 过滤 history + Pareto filter + sort/limit + 加载 layer_profile.json
- **Execute verifications** (`execute_verifications`, sandbox): 跑 agent 写的
  Python, 把 query results 通过 argv[2] JSON 传进去
- **Stage B-2** (`invoke_scientist_stage_b2`): 综合 drafts + query results +
  verification results + annotations 写最终 insights.md
- **Pipeline** (`scientist_pipeline`): backup → A → B-1 → resolve → exec → B-2 →
  atomic write; 每阶段失败独立 fallback (Stage A 失败不写, B-1/B-2 失败用 drafts
  渲染兜底), 总失败也不阻断 NSGA-II
- **Helpers**: `read_insights_md`, `write_insights_md_atomic` (tempfile +
  os.replace), `backup_insights_md` (snapshot 成 `insights.md.gen{N}.bak`)
- 35/35 测试通过 (含 _matches_pattern 各种 case, resolve_vela_queries 各种
  filter, 三阶段 mocked LLM, 全 pipeline 成功路径 + 三种失败路径)

### 3.3 Scientist Prompts ✅

`prompts.py` 新增三段:
- `SCIENTIST_STAGE_A_SYSTEM` (4080 chars): 纯归纳 + 显式禁止引用硬件 cycles /
  写 Python; 解释为什么分阶段 (防 hardware-verifiable bias)
- `SCIENTIST_STAGE_B1_SYSTEM` (5296 chars): 完整的 vela_query schema (null
  通配符 / by_arch_codes / Pareto filter / sort_by_* / limit) + 代码沙箱规则
  (白名单 / 30s 超时 / argv 接口)
- `SCIENTIST_STAGE_B2_SYSTEM` (4564 chars): grounding annotation 的写法 +
  status 调整规则 + 兜底处理 (timeout / 0 matches 等情况)

### 3.4 NSGA-II Hook + Wrapper ✅

`nsga2_search.py`:
- `NSGA2SearchRunner.__init__` 新增 `scientist_llm`, `scientist_interval` (默认 3),
  `scientist_sandbox_timeout` (默认 30)
- `_maybe_invoke_scientist(generation)`: 在 `(generation+1) % K == 0` 时调
  scientist_pipeline; 任何异常都被捕获不阻断 NSGA-II
- 触发时机: K=3 → gen 2/5/8/11/14 后各触发一次, 16 代共 5 次反思

`run_nsga2_search.py`:
- 加 `--enable_scientist`, `--scientist_interval` (默认 3),
  `--scientist_sandbox_timeout` (默认 30) flags
- 复用 warmstart 创建的 LLMClient (如果 --enable_warmstart 也启用)

### 3.5 Tests ✅

- `tests/test_sandbox.py`: 18 个 (validate_imports + execute_python + execute_verification)
- `tests/test_scientist_agent.py`: 35 个 (mocked LLM + 真实 sandbox + 真实 file_io;
  覆盖 _matches_pattern, resolve_vela_queries 各类 filter, 三阶段独立 + 完整 pipeline 成功 / 三种失败路径)
- 联合套件 169/169 (Phase 1+2+3 相关) passed
- 全套 301 tests, 1 unrelated error (reportlab 缺包)

### Ablation 设计 (Phase 5 跑)

- (a) NSGA-II only
- (b) NSGA-II + Warmstart
- (c) NSGA-II + Warmstart + Scientist
- 每次跑产出 `insights.md.gen{N}.bak` 系列, Phase 5 可以分析 insights 演化
  + grounding 命中率 + verification code 跑通率

## Phase 4: Supervisor Agent ✅ Complete (2026-05-05)

**最终设计: 5-lever 动作空间 + 无策略边界 + 不做自动 rollback** —— 偏离原 outline
里的"只调 mutation_prob + 硬边界 + 自动 rollback"。用户两次关键 push:
1. 动作空间应该匹配 Phase 1.3 的 8 类指标，而不是只调 mutation_prob 的单维标量
2. agent 知道 current_state + supervisor_log 就能自我纠偏，硬边界是替 agent 担心
3. ablation 不需要 rule-based 对照，直接 (d) vs (c) 测增量就够诚实

### 4.1 NSGA-II 5-Lever Surgery ✅

`efnas/baselines/nsga2_search.py`:
- `mutate_arch` 加 `per_dim_multiplier` kwarg, 每维独立 mutation 缩放
- `_tournament_select_index` 用 `self._tournament_size` (默认 2, 可调到
  population_size) 替代硬编码 K=2
- `_run_single_generation` 加 reseed injection: `_reseed_bottom_pct` > 0 时
  从 needed 里切出 N 个 random arch 注入 offspring batch
- `NSGA2SearchRunner.__init__` 加 mutable state: `_tournament_size`,
  `_per_dim_mutation_multiplier`, `_reseed_bottom_pct`, `_supervisor_llm`
- 新方法 `current_supervisor_state` (返回 5 lever 当前值) 和
  `apply_supervisor_actions` (合法性校验 + 应用)

### 4.2 Supervisor Agent 模块 ✅

`efnas/search/supervisor_agent.py`:
- `invoke_supervisor_agent` 单次 LLM 调用, 返回 actions /
  rationale / expected_effect / review_after_gen
- `read_supervisor_log` / `append_supervisor_log` —— supervisor_log.json
  读写, agent 下次能看到自己历史
- `_summarize_current_pareto` 给 agent 看的 Pareto 前沿端点摘要
- `supervisor_pipeline` orchestrator: 准备输入 → 调 LLM → 应用 actions →
  写日志; LLM 失败也写一条 "_global rejected" 日志

### 4.3 SUPERVISOR_AGENT_SYSTEM Prompt ✅

`prompts.py` 新增段:
- 5 levers 各自的语义 + 数学合法范围
- Diagnostic signals 清单 (纯诊断, 不写 if-then)
- Output schema
- "no_change 是合理输出" 的反复强调
- 风险红线提醒 (Phase 4 允许被消融出局)

### 4.4 NSGA-II Hook + Wrapper ✅

`nsga2_search.py`:
- `_maybe_invoke_supervisor` 在 `_maybe_invoke_scientist` 之后调用 (Scientist
  之后立即 Supervisor, 让它能读 fresh insights)
- 触发频率与 Scientist 同步 (`(gen+1) % scientist_interval == 0`)

`run_nsga2_search.py`:
- `--enable_supervisor` flag
- `--supervisor_role` (默认 "supervisor_agent")
- 复用 LLMClient (warmstart / scientist 已 build 过的话)

### 4.5 Tests ✅

- `tests/test_supervisor_agent.py` (26 tests):
  - mutate_arch with per_dim_multiplier (zero / high / None fallback)
  - current_supervisor_state 默认值 + 应用后变化
  - apply_supervisor_actions 各种合法 / 非法值的处理 (10 tests)
  - tournament_size live behavior
  - supervisor_log read/write 含损坏文件
  - invoke_supervisor_agent (mocked LLM, 含失败兜底)
  - supervisor_pipeline 端到端 (success / LLM failure / no_change /
    部分 rejected)
- 联合套件 195/195 (Phase 1-4) passed
- 全套 327 tests, 1 unrelated error (reportlab)

### Ablation 红线 (Phase 5 跑后)

任一发生则 Phase 4 应被消融出局:
- (d) HV ≤ (c) HV (Supervisor 无贡献)
- (d) HV < (c) HV (Supervisor 有害)
- (d) 多 seed 方差显著大于 (c) (引入有害不稳定性)

论文叙事上"我们尝试了 Phase 4, 数据显示无显著贡献, 已移除" 比硬撑弱
对照可信得多。

## Phase 5: Validation & Paper Artifacts ✅ Ready (2026-05-05)

**用户决策简化**: 单 seed × 4 组, (a) 复用 search_v3 distill run, 仅 3 个新
slurm × 3h ≈ ~9h 墙钟. LLM 全栈 Gemini 3.1 Pro.

### 5.1 LLM Config + Role Map ✅

- `efnas/search/llm_client.py`: `ROLE_TO_CONFIG_KEY` 重写为 Phase 2-4 的 5 个
  role (warmstart_agent / scientist_stage_a/b1/b2 / supervisor_agent), 删除
  旧 agent_a/b/c/d 间接映射
- `configs/nsga2_v3.yaml`: 新增 `llm:` 段, 5 role 路由到
  `gemini/gemini-3.1-pro-preview`, temperature/max_tokens/timeout 配齐
- `tests/test_llm_json_retry.py`: role 名更新 (warmstart_agent 替代 agent_b)

### 5.2 Wrapper `--ablation_group` 便捷 flag ✅

`wrappers/run_nsga2_search.py`:
- `--ablation_group {a,b,c,d}`: 自动展开成 enable_warmstart/scientist/supervisor
  组合 (a=无 / b=warmstart / c=+scientist / d=完整)
- `--output_root` CLI 覆盖 (用于把 outputs/ 路由到
  `outputs/ablation_phase5/`)

### 5.3 Slurm 脚本 ✅

`plan/ablation_phase5/run_group_{b,c,d}.slurm` 三个文件:
- 4 P100 / 24 CPU / 160G / 5h 超时
- 模板对齐 `distill_or_not_fc2_short_4gpu.slurm`
- 提交前检查 `GEMINI_API_KEY` 环境变量
- 用 V3 distill checkpoint
  (`edgeflownas_supernet_v3_fc2_172x224_run1_archparallel_distill`)

### 5.4 Ablation 分析脚本 ✅

`wrappers/run_phase5_ablation_analysis.py`:
- 从 4 组 history_archive.csv 加载数据
- 计算各预算点 (50/100/200/400/600/800 evals) 的 HV / best_epe / best_fps /
  pareto_count
- 红线判定: phase2/3/4 各阶段是否 PASS
- 输出: `summary.json` + `hv_trajectory.png` + `pareto_fronts.png` + `report.md`
- Smoke-tested on 现有 distill run: (a) final HV = 7.5217 ✅

### 5.5 Phase 5 文档 ✅

`plan/search_hybrid_v1/phase5_ablation.md`:
- 提交前 checklist (GEMINI_API_KEY / supernet 路径 / LLM 配置)
- 提交命令 (sbatch × 3)
- 监控指引 + resume 指令
- 红线判定 + 论文叙事原则
- 预期结果区间
- Troubleshooting (rate limit / sandbox timeout / 等)

### 提交后 (HPC 端) 工作流

```bash
sbatch plan/ablation_phase5/run_group_b.slurm   # ~3h
sbatch plan/ablation_phase5/run_group_c.slurm   # ~3h
sbatch plan/ablation_phase5/run_group_d.slurm   # ~3h
# 三个 slurm 顺序排队 (4 GPU 限制), 总 ~9h

# 跑完后:
python wrappers/run_phase5_ablation_analysis.py
# → outputs/ablation_phase5/analysis_<timestamp>/{summary.json, *.png, report.md}
```

### 红线判定 (摘自 phase5_ablation.md)

| 比较 | 期望 | 触发 FAIL 行动 |
|---|---|---|
| HV(b) ≥ HV(a) | Warmstart 有贡献 | 移除 Phase 2 |
| HV(c) ≥ HV(b) | Scientist 有贡献 | 移除 Phase 3 |
| HV(d) ≥ HV(c) | Supervisor 有贡献 | **强烈建议**移除 Phase 4 |
