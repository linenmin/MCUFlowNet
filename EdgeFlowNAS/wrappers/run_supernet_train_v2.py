"""Supernet V2 training CLI wrapper."""
...
from efnas.app.train_supernet_app_v2 import run_supernet_app_v2
from wrappers.run_supernet_train import _build_overrides


def _build_parser() -> argparse.ArgumentParser:
    """Build CLI parser."""
    parser = argparse.ArgumentParser(description="run supernet v2 train locally without docker")
    parser.add_argument("--config", default="configs/supernet_fc2_180x240_v2.yaml", help="path to supernet v2 config yaml")
...
def main() -> int:
    ...
    result = run_supernet_app_v2(config_path=args.config, overrides=overrides, dry_run=args.dry_run)
    if args.dry_run:
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0

    print(f"Supernet V2 training finished with exit_code={result.get('exit_code', 1)}")
