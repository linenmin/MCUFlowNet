"""CLI wrapper for timing one V2 subnet on FC2 val versus Sintel."""
...
from efnas.nas.supernet_v2_timing_probe import run_timing_probe
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="measure FC2-val versus Sintel timing for one inherited-weight supernet_v2 subnet")
    ...
    parser.add_argument("--arch_code", default=None, help="fixed V2 arch code, default uses the V2 reference arch")
    parser.add_argument("--output_dir", default=None, help="where to write timing outputs")
    ...
    parser.add_argument("--bn_recal_batches", type=int, default=16, help="FC2-train BN recalibration batches")
    ...
    parser.add_argument("--dry_run", action="store_true", help="print merged config preview and exit")
    return parser
...
def main() -> int:
    ...
    result = run_timing_probe(config_path=args.config, overrides=overrides, options=options)
