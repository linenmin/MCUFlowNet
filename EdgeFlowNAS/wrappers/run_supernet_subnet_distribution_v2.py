"""Supernet V2 fixed-subnet evaluation wrapper for agentic search."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="run fixed-arch V2 subnet evaluation without editing yaml")
    ...
    parser.add_argument(
        "--fixed_arch",
        default=None,
        help="evaluate a single fixed V2 arch code, e.g. '0,1,2,0,1,2,0,1,0,1,0'",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="deprecated and ignored; FC2 val uses full sequential coverage unless --max_fc2_val_samples is set",
    )
...
def _resolve_entry_module() -> str:
    """Resolve callable module path with backward compatibility."""
    candidates = [
        "efnas.nas.supernet_subnet_distribution_v2",
    ]
    for module_name in candidates:
        try:
            ...
            continue
    raise ModuleNotFoundError("Cannot resolve V2 subnet distribution module.")
