...
def load_candidates(csv_path: Path) -> List[Dict[str, object]]:
    ...
    default_path = _project_path("configs/retrain_candidates.csv")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--candidates_csv", default="configs/retrain_candidates.csv")
...
def _child_command(args, candidate: Dict[str, object]) -> List[str]:
    ...
    config = args.config or ("configs/retrain_fc2.yaml" if args.stage == "fc2" else "configs/retrain_ft3d.yaml")
    cmd = [
        sys.executable,
