...
def _build_fairness_counts(epoch_count: int, num_blocks: int = 9) -> Dict[str, Dict[str, int]]:  # 定义公平计数构建函数
    """构建占位公平计数字典。"""  # 说明函数用途
    counts: Dict[str, Dict[str, int]] = {}  # 初始化计数字典
    for block_idx in range(num_blocks):  # 按block数量循环
        counts[str(block_idx)] = {"0": epoch_count, "1": epoch_count, "2": epoch_count}  # 写入三选项等计数
...
def train_supernet(config: Dict[str, Any]) -> int:  # 定义超网训练函数
    ...
    for epoch_idx in range(1, total_epochs + 1):  # 按轮数循环训练
        train_stats = run_train_step(epoch_idx=epoch_idx)  # 执行占位训练单步
        ...
        row = {  # 组装单轮评估记录
            ...
            "fairness_gap": float(eval_stats["fairness_gap"]),  # 写入公平差距
    ...
    fairness_counts = _build_fairness_counts(epoch_count=len(eval_rows))  # 构建公平计数结果
    write_json(str(experiment_dir / "fairness_counts.json"), fairness_counts)  # 写入公平计数文件
    write_json(str(experiment_dir / "eval_pool_12.json"), {"pool_size": int(eval_cfg.get("eval_pool_size", 12))})  # 写入占位验证池文件
