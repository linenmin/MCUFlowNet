...
def _build_graph(config: Dict[str, Any]) -> Dict[str, object]:  # 定义训练图构建函数
    ...
    model = MultiScaleResNetSupernet(  # 创建超网模型实例
        ...
        num_out=2,  # 设置输出通道为2
        init_neurons=32,  # 设置初始通道为32
        expansion_factor=2.0,  # 设置扩展倍率为2.0
    )
    ...
    for idx, (grad, var) in enumerate(zip(clipped, vars_)):  # 遍历裁剪梯度和变量
        accum_var = tf.compat.v1.get_variable(  # 创建梯度累积变量
            name=f"strict_accum_{idx}",  # 设置累积变量名称
            shape=var.shape,  # 设置累积变量形状
            dtype=var.dtype.base_dtype,  # 设置累积变量类型
            initializer=tf.zeros_initializer(),  # 使用零初始化
            trainable=False,  # 设置为不可训练变量
        )
    ...
    return {  # 返回图对象字典
        ...
        "lr_ph": lr_ph,  # 返回学习率占位符
        "preds": preds,  # 返回多尺度预测张量
        "loss": loss_tensor,  # 返回损失张量
        "epe": epe_tensor,  # 返回EPE指标张量
        "global_grad_norm": global_norm,  # 返回全局梯度范数张量
        "zero_ops": zero_ops,  # 返回累积清零操作
        "accum_op": accum_op,  # 返回累积执行操作
        "apply_op": apply_op,  # 返回参数更新操作
        "saver": saver,  # 返回检查点管理器
    }
...
def _run_eval_epoch(  # 定义评估轮执行函数
    ...
    for arch_code in eval_pool:  # 遍历验证子网架构编码
        run_bn_recalibration_session(  # 执行BN统计重估
            sess=sess,  # 传入当前会话
            forward_fetch=graph_obj["preds"][-1],  # 传入最终预测张量
            input_ph=graph_obj["input_ph"],  # 传入输入占位符
            label_ph=graph_obj["label_ph"],  # 传入标签占位符
            arch_code_ph=graph_obj["arch_code_ph"],  # 传入架构编码占位符
            is_training_ph=graph_obj["is_training_ph"],  # 传入训练标志占位符
            batch_provider=train_provider,  # 传入训练采样器
            arch_code=arch_code,  # 传入当前架构编码
            batch_size=batch_size,  # 传入批大小
            recal_batches=bn_recal_batches,  # 传入重估批次数
        )
        ...
        epe_val = sess.run(  # 执行EPE推理
            graph_obj["epe"],  # 抓取EPE张量
            feed_dict={  # 传入推理喂入字典
                graph_obj["input_ph"]: input_batch,  # 传入输入批数据
                graph_obj["label_ph"]: label_batch,  # 传入标签批数据
                graph_obj["arch_code_ph"]: arch_code,  # 传入架构编码
                graph_obj["is_training_ph"]: False,  # 关闭训练模式
            },
        )
        ...
        for epoch_idx in range(1, num_epochs + 1):  # 按轮数执行训练
            for _ in range(steps_per_epoch):  # 按每轮步数执行迭代
                ...
                for arch_code in cycle_codes:  # 遍历周期内3条路径
                    sess.run(  # 执行单路径梯度累积
                        graph_obj["accum_op"],  # 执行累积操作
                        feed_dict={  # 传入累积喂入字典
                            graph_obj["input_ph"]: input_batch,  # 传入输入批数据
                            graph_obj["label_ph"]: label_batch,  # 传入标签批数据
                            graph_obj["arch_code_ph"]: arch_code,  # 传入当前路径架构编码
                            graph_obj["is_training_ph"]: True,  # 开启训练模式
                            graph_obj["lr_ph"]: current_lr,  # 传入当前学习率
                        },
                    )
                loss_val, grad_norm_val, _ = sess.run(  # 执行参数更新并抓取指标
                    [graph_obj["loss"], graph_obj["global_grad_norm"], graph_obj["apply_op"]],  # 指定抓取张量
                    feed_dict={  # 传入更新喂入字典
                        graph_obj["input_ph"]: input_batch,  # 传入输入批数据
                        graph_obj["label_ph"]: label_batch,  # 传入标签批数据
                        graph_obj["arch_code_ph"]: cycle_codes[0],  # 传入占位架构编码
                        graph_obj["is_training_ph"]: True,  # 开启训练模式
                        graph_obj["lr_ph"]: current_lr,  # 传入当前学习率
                    },
                )
                global_step += 1  # 递增全局步计数
            eval_info = _run_eval_epoch(  # 执行整轮评估
                sess=sess,  # 传入当前会话
                graph_obj=graph_obj,  # 传入图对象字典
                train_provider=train_provider,  # 传入训练采样器
                val_provider=val_provider,  # 传入验证采样器
                eval_pool=eval_pool,  # 传入固定验证池
                bn_recal_batches=bn_recal_batches,  # 传入BN重估批次数
                batch_size=batch_size,  # 传入批大小
            )
            row = {  # 组装评估行记录
                "epoch": int(epoch_idx),  # 写入当前轮数
                "mean_epe_12": float(eval_info["mean_epe_12"]),  # 写入平均EPE
                "std_epe_12": float(eval_info["std_epe_12"]),  # 写入EPE标准差
                "fairness_gap": float(_fairness_gap(fairness_counts)),  # 写入公平差距
                "lr": float(cosine_lr(base_lr=base_lr, step_idx=global_step, total_steps=total_steps)),  # 写入下轮学习率
                "bn_recal_batches": float(bn_recal_batches),  # 写入BN重估批次数
            }
            ...
            logger.info(  # 记录轮级日志
                "epoch=%d loss=%.6f mean_epe_12=%.6f std_epe_12=%.6f fairness_gap=%.2f grad_norm=%.4f",  # 定义日志模板
                epoch_idx,  # 传入当前轮数
                float(loss_val),  # 传入当前损失
                row["mean_epe_12"],  # 传入平均EPE
                row["std_epe_12"],  # 传入EPE标准差
                row["fairness_gap"],  # 传入公平差距
                float(grad_norm_val),  # 传入梯度范数
            )
