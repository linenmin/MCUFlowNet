"""Fixed-architecture V2 retrain model with supernet-compatible variable names."""

from typing import Dict, Sequence

import tensorflow as tf

from efnas.nas.search_space_eval import validate_arch_code
from efnas.network.base_layers import BaseLayers


class FixedArchModelV2(BaseLayers):
    """Build one fixed V2 subnet while preserving supernet branch variable names."""
    ...
    def _deep_choice_block(self, inputs, filters, choice_idx, name):
        ...
        with tf.compat.v1.variable_scope(name):
            if choice == 0:
                return self._res_block(inputs=inputs, filters=filters, name="branch1_block1")
            if choice == 1:
                out = self._res_block(inputs=inputs, filters=filters, name="branch2_block1")
                return self._res_block(inputs=out, filters=filters, name="branch2_block2")
            out = self._res_block(inputs=inputs, filters=filters, name="branch3_block1")
            out = self._res_block(inputs=out, filters=filters, name="branch3_block2")
            return self._res_block(inputs=out, filters=filters, name="branch3_block3")
    ...
    def _e0_choice_block(self, inputs, filters, choice_idx, name):
        ...
        with tf.compat.v1.variable_scope(name):
            if choice == 0:
                return self.conv_bn_relu(inputs=inputs, filters=filters, kernel_size=(7, 7), strides=(2, 2), name="k7")
            if choice == 1:
