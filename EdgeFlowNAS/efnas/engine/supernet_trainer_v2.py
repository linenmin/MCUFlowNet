...
from efnas.nas.search_space_v2 import (
    V2_REFERENCE_ARCH_CODE,
    get_block_name,
    get_block_specs,
    get_num_blocks,
    get_num_choices,
...
def _build_resume_signature_v2(config: Dict[str, Any]) -> Dict[str, Any]:
    """Build V2 resume-critical signature."""
...
def _build_run_manifest_v2(config: Dict[str, Any], git_commit: str) -> Dict[str, Any]:
    """Build V2 run manifest."""
    ...
    return {
        ...
        "resume_signature": _build_resume_signature_v2(config=config),
        "train_snapshot": {
...
def _build_manifest_v2(config: Dict[str, Any], git_commit: str) -> Dict[str, Any]:
    """Build V2 legacy train manifest payload."""
    train_cfg = config.get("train", {})
    runtime_cfg = config.get("runtime", {})
    data_cfg = config.get("data", {})
    return {
...
def _init_fairness_counts() -> Dict[str, Dict[str, int]]:
    """Initialize V2 fairness counters."""
...
def _parse_teacher_arch_code(raw_value: Any) -> List[int]:
    """Parse V2 teacher arch code with fallback to V2 reference code."""
    default_code = [int(item) for item in V2_REFERENCE_ARCH_CODE]
...
def _build_graph(config: Dict[str, Any]) -> Dict[str, object]:
    """Build TF1 supernet V2 training graph."""
    train_cfg = config.get("train", {})
    ...
    if distill_enabled:
        with tf.compat.v1.variable_scope("teacher"):
            if distill_teacher_type == "supernet":
                ...
                teacher_model = MultiScaleResNetSupernetV2(
                    input_ph=input_ph,
...
def _validate_resume_run_manifest(config: Dict[str, Any], resume_dir: Path, logger) -> None:
    """Validate resume checkpoint compatibility with V2 run manifest."""
    ...
    current_manifest = _build_run_manifest_v2(config=config, git_commit=_git_commit_hash())
...
def train_supernet(config: Dict[str, Any]) -> int:
    ...
    run_manifest = _build_run_manifest_v2(config=config, git_commit=_git_commit_hash())
    ...
    logger.info("supernet V2 training finished")
    return 0
