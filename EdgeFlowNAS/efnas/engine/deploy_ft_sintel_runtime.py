...
V3 candidates go through the existing `distill_or_not_sintel_runtime`
helper. Mainline (EdgeFlowNet original) needs its own setup since the
graph topology and the (no) flow_divisor differ.
...
from efnas.engine.distill_or_not_sintel_runtime import (
    evaluate_v3_checkpoint_dir_on_sintel,
)
...
def evaluate_mainline_checkpoint_on_sintel(
    ...
    used for V3 (FlowPostProcessor, no flow_scale)."""
    ...
    try:
        for idx in iterator:
            ...
            # Mainline: NO flow_scale multiplier (training never divided GT).
            flow_prediction = preds[:, :, :, :2]
    ...
    mean_epe = (
        float(np.concatenate(error_epes).mean())
        if error_epes
        else float("inf")
    )
    return {
