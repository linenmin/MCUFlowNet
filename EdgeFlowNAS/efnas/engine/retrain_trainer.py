...
def train_retrain_v3(config: Dict[str, Any]) -> int:
    ...
    write_json(
        str(manifest_path),
        {
            ...
            "dataset": dataset,
            "scratch_init": not checkpoint_cfg.get('load_checkpoint', False) and str(checkpoint_cfg.get("init_mode", "none")).strip().lower() in ("", "none"),
            "model_family": 'component_ablation' if config.get('component_variant') else 'fixed_v3',
            "config": config,
