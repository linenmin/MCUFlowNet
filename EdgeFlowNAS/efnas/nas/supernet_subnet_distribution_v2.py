...
class _FixedSubnetForExportV2(BaseLayers):
    """Build a hard-routed V2 subnet graph for TFLite/Vela export only."""
    ...
    def _res_block(self, inputs, filters, name):
        """Residual block with checkpoint-compatible variable names."""
    ...
    def _deep_choice_block(self, inputs, filters, choice_idx: int, name: str):
        """Instantiate all depth branches, then return the selected one."""
    ...
    def _conv_bn_relu_dilated(self, inputs, filters, kernel_size, dilation_rate, name):
        """One dilated conv branch with checkpoint-compatible variable names."""
        import tensorflow as tf
    ...
    def _e0_choice_block(self, inputs, filters, choice_idx: int, name: str):
        """Stem E0 choice: instantiate all branches, route one by python."""
        choice = int(choice_idx)
        if choice not in (0, 1, 2):
    ...
    def _e1_choice_block(self, inputs, filters, choice_idx: int, name: str):
        """Stem E1 choice: instantiate all branches, route one by python."""
        choice = int(choice_idx)
        if choice not in (0, 1, 2):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            return [conv5, conv3, dilated][choice]

    def _head_choice_conv(self, inputs, filters, choice_idx: int, name: str):
        """Head conv choice: 3x3 / 5x5."""
        choice = int(choice_idx)
        if choice not in (0, 1):
            raise ValueError(f"invalid head conv choice: {choice_idx}")
        import tensorflow as tf
    ...
    def _head_choice_resize_conv(self, inputs, filters, choice_idx: int, name: str):
        """Head upsample choice: 3x3 / 5x5."""
        choice = int(choice_idx)
        if choice not in (0, 1):
            raise ValueError(f"invalid head resize choice: {choice_idx}")
        import tensorflow as tf
    ...
    def build(self):
        """Build fixed V2 subnet graph with checkpoint-compatible scopes."""
