...
from efnas.utils.json_io import write_json  # 自定义 JSON 写入工具

try:
    from efnas.network.base_layers import BaseLayers  # 复用基础层定义
...
def _evaluate_arch_pool(
    ...
    from efnas.data.dataloader_builder import build_fc2_provider  # 导入数据提供器构建函数
    from efnas.nas.supernet_eval import (
...
def _run_vela_for_arch(
    ...
    def _resolve_run_vela() -> Callable[..., Any]:
        ...
        try:
            from efnas.vela.vela_compiler import run_vela as imported_run_vela
            ...
            if not vela_file.exists():
                raise ImportError(
                    "failed to import efnas.vela.vela_compiler.run_vela and fallback file not found: "
                    f"{vela_file}"
                ) from import_exc
    ...
    try:
        ...
        raise ImportError(  # 导入失败时抛出更明确的错误
            "failed to import efnas.vela.vela_compiler.run_vela; "
            "please ensure EdgeFlowNAS/efnas/vela exists"
        ) from exc
