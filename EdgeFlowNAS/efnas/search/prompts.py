...
你的真正任务是对比评估报告中，源于**不同积木深度和感受野大小配置层**（如 Encoder/Decoder blocks 或 Head conv 层）所引发的异常（如算子无法跑满 NPU、Util% 极低或 Op Cycles 占比异常飙升）。
...
2. 脚本应当依靠 `pandas` 极快地筛查 CSV。
   【致命代码规范】：CSV 中的 `arch_code` 列是形如 `"0,1,2,0,0,1,2,1,0"` 的带逗号文本。在执行数组维度切片或位置判定时，**绝对禁止直接用字符串下标 (如 `x[6]`)**！这是物理错误！**你必须使用 `.str.split(',').str[idx]` 或 `lambda x: x.split(',')[idx]` 来安全地获取对应的第 N 维度的真实值。**
...
【极其重要的防冗余与合并约束】：
在写入前，请极其严格地对比已有规则！如果新真理（如判定条件为“第8位是2”）与旧真理（如“第7或第8位是2”）在物理判断空间上存在严重重叠、或者是旧规则的子集/超集，**你必须重写合并且取判断力最合理的并集**，禁止产生语义冗余的垃圾规则导致库臃肿！
