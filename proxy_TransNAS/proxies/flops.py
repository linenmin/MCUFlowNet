...
def compute_flops(model, train_batches, device, decoder_only: bool = False):  # 计算 FLOPs；decoder_only 仅累计解码器模块
    model.eval()  # 评估模式
    ...
    with torch.no_grad():  # 关闭梯度
        with warnings.catch_warnings():  # 捕获警告
            ...
            if decoder_only:
                by_mod = flop_analyzer.by_module()
                total_flops = sum(v for k, v in by_mod.items() if "decoder" in str(k))
            else:
