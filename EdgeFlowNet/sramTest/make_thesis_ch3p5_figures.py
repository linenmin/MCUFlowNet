...
# encoder/decoder split: the decoder begins at the first upsampling operator
bnd = ops.index("TRANSPOSE_CONV") - 0.5
...
# encoder / decoder demarcation
ax.axvspan(-0.5, bnd, color=COLOR_CONV, alpha=0.06, zorder=0)
ax.axvspan(bnd, len(df) - 0.5, color=COLOR_TCONV_PEAK, alpha=0.06, zorder=0)
ax.axvline(bnd, color="#555555", linestyle="--", linewidth=0.8)
ax.text((-0.5 + bnd) / 2, peak_val * 1.23, "Encoder",
        ha="center", va="top", fontsize=9, style="italic", color="#333333")
ax.text((bnd + len(df) - 0.5) / 2 + 3.0, peak_val * 1.23, "Decoder",
        ha="center", va="top", fontsize=9, style="italic", color="#333333")

ax.set_xlabel("Layer index (execution order)")
ax.set_ylabel("SRAM usage (KiB)")
ax.set_xticks(range(len(df)))
ax.set_xticklabels([str(i + 1) for i in range(len(df))], fontsize=7)
ax.set_ylim(0, peak_val * 1.32)
