...
tflite_output_path = os.path.join(current_dir, "sram_test_modified.tflite")
...
def main():
    global height, width, output_dir, tflite_output_path

    parser = argparse.ArgumentParser(description="Original EdgeFlowNet SRAM test exporter")
    ...
    output_dir = os.path.join(current_dir, f"output_original_{size_suffix}")
    os.makedirs(output_dir, exist_ok=True)
    tflite_output_path = os.path.join(current_dir, f"sram_test_modified_{size_suffix}.tflite")
