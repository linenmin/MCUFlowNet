...
ax_top.set_xticks(range(len(df_a0)))
ax_top.set_xticklabels([str(i + 1) for i in range(len(df_a0))], fontsize=7)
...
# headroom: the vertical room between the binding peak and the low layers, which
# is the SRAM budget that the attention modules of Section 4.3 spend (see text)
enc_end = list(df_a1["TFLite_operator"]).index("RESIZE_BILINEAR")  # first decoder op
bn_idx  = enc_end - 2                       # deepest encoder convolution (ECA host)
bn_val  = float(a1_kib.iloc[bn_idx])
ax_bot.annotate(
    "", xy=(bn_idx, a1_peak_val), xytext=(bn_idx, bn_val),
    arrowprops=dict(arrowstyle="<->", color="#333333", lw=1.1),
)
ax_bot.text(
    bn_idx + 1.4, (bn_val + a1_peak_val) / 2.0,
    "vertical headroom\nfor added capacity",
...
ax_bot.set_xlabel("Layer index (execution order)")
ax_bot.set_ylabel("SRAM usage (KiB)")
ax_bot.set_xticks(range(len(df_a1)))
ax_bot.set_xticklabels([str(i + 1) for i in range(len(df_a1))], fontsize=7)
...
axS.plot(
    ...
    linewidth=1.8, markersize=6, label="Bilinear resize+conv (A1)",
)
