...
`tools/baselines/export_grove.py`恢复公开权重，使用固定64对FC2训练图像校准，保存浮点和严格INT8 TFLite，再运行Vela4.5.0/U55-64的Size与Performance。TF模型沿用已有WSL容器；输出目录必须不存在。脚本参数包括模型、权重、上游目录、校准目录、输入高宽、Vela配置/安装目录及输出目录。默认在各尺度累加前只保留UV，不计算辅助输出的累加，但预测卷积仍有4个输出通道；新增可选的`--prune-aux-heads`仅用于S/L对照，将这些卷积的最后两个滤波器也去掉，并核对浮点结果。该选项不改变默认导出。
...
diagnose_grove_accuracy.py直接读取既有导出，默认每场景取首、中、末三对（共69对）。分别测浮点、完整INT8、仅输入量化、仅最终输出量化，保存运动大小分组和中间层误差。所有误差都还原到共同的416×1024像素单位；抽样结果不替代1041对总表。可追加--highres-weights和--upstream，从同一权重恢复416×1024原生浮点图，比较原图与“先缩小再放大”的输入；此路径仅用于诊断信息损失和工作尺度差异，不代表可部署实现。

示例（在既有容器内，挂载方式沿用上述导出）：

    python tools/baselines/diagnose_grove_accuracy.py \
      --export /runs/GROVE-INT8-01/scan/MCUFlowNet-S/160x224 \
      --dataset /datasets/Sintel \
