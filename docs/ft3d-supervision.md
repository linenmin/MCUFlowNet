...
2026-09-24 authorization update: the user approved overnight continuation of both FT3D-CONTEXT-01 runs to60000. Queue each continuation after successful completion of its20k job, using `--action continue --stop-step 60000` with the same recipe/code. Preserve full state and original60k cosine. The20k checkpoint remains; no manual review gate is required for this authorized continuation.
