...
若旧最佳检查点仅剩元信息、缺少index/data，诊断清单明确记录缺失，只对仍完整的保存点评测，不用另一轮次冒充。2026-09-19发现V3训练保存器的`max_to_keep=3`小于四类命名保存点的数量；旧最佳可能被自动清理，JSON元信息仍残留。下一次训练前必须修复并验证四类保存点跨进程继续保存后均可恢复；本次诊断没有修改训练保存逻辑。已有缺失权重不能由成绩JSON恢复。
