...
## FT3D长周期与初始学习率：FT3D-SCHEDULE-01

配置为`EdgeFlowNAS/configs/experiments/ft3d_schedule.json`。S/L各从FC2第150轮末尾权重初始化，保留模型和BN、新建Adam，训练60000步。`s_slow/l_slow`的LR为1e-5→1e-6；`s_higher/l_higher`为3e-5→1e-6，均使用完整60000步余弦周期。其余数据、batch、裁剪、增强、种子和监控设置沿用150轮参照。40/80/120个报告区间保存完整快照，即20000/40000/60000步；暂停或恢复不重新计算周期。

启动前用`tools/validation/check_saved_sintel.py --model-dir <原150轮起点FT3D模型目录> --expected-step 20000 --output <独立验分目录>`核对S/L已保存权重。此工具在新进程建立推理图、严格加载模型/BN、评分固定845对，原始GT和历史GT与日志偏差都须≤1e-5，且确认权重指纹未变化。验分通过后，四组仍先运行独立100步短跑（50步退出再恢复）；检查配对输入、初始化、恢复、LR及快照，再启动正式训练。

正式每1000步快速观察、每5000步完整监控，20000和40000步做自动完整性检查后继续至预先批准的60000步。比较同预算节点，并另比较长周期最后50000/55000/60000步均值与既有短周期末三点均值，明确后者计算量不同。筛选改善≥0.05且多个节点方向一致的候选；独立种子复验另行决定。完整运行记录和权重留在Runs，状态仅记录wiki实验总表。
