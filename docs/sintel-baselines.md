...
“未用Sintel微调”仅说明没有用它作梯度训练，并不证明从未用它选模。已有MCUFlowNet论文权重是sintel_best，明确用过Sintel选模，因此不应把这张对比表整体称为严格零样本测试。S/L复用2026-09-17已完成测量；汇总器的`--mcu-reference`可核对旧逐图CSV与本次样本清单完全一致，再引入参考成绩，标明不是本轮重新推理。
