...
class TransBench101SearchSpaceMicro(Graph):
    ...
    def _create_graph(self):
        ...
        # node variable leaks from previous loops, pointing to the last module node
        n_channels_decoder = self._get_module_n_output_channels(module)
        
        self.edges[node, node + 1].set('op',
                                       self._get_decoder_for_task(self.dataset,
                                                                  n_channels=n_channels_decoder)
                                       )
    ...
    def _get_decoder_for_task(self, task, n_channels):
        if task == "jigsaw":
            ...
            # 修复：使用传入的 n_channels，而不是硬编码的 64 或 512
            # 同时保留原有的 output_shape (256, 2048) 配置
            input_size = 32 if self.use_small_model else 16
            return ops.GenerativeDecoder((n_channels, input_size), (256, 2048))

        # === 修复 segmentsemantic 的 Decoder ===
        elif task == "segmentsemantic":
            input_size = 32 if self.use_small_model else 16
            
            # 修复：正确构建分割解码器，使用动态 n_channels 和 1x1 卷积映射到 17 类
            if self.use_small_model:
                model = ops.Sequential(
                    ops.GenerativeDecoder((n_channels, input_size), (256, 2048)), 
                    nn.Conv2d(3, 17, kernel_size=1) 
                )
            else:
                model = ops.Sequential(
                    ops.GenerativeDecoder((n_channels, input_size), (256, 2048)),
    ...
    def _set_cell_ops_for_module(self, module, C_in, stage):
        ...
        for idx, cell in enumerate(cells):
            ...
            # 修复：使用 lambda 默认参数捕获循环变量 C_in 和 downsample 的当前值，避免 Python 闭包延迟绑定导致的错误
            cell.update_edges(
                update_func=lambda edge, c=C_in, d=downsample: _set_op(edge, c, d),
